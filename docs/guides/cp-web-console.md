...
**Status:** Map B thin demo — optional Bearer auth + SQLite registry.

```bash
cd <app-with-.rn/delivery>
RN_CP_REGISTRY=sqlite RN_CP_TOKEN=your-secret rn-delivery serve --port 4040
...
Verify: `node scripts/verify-cp-stub-api.mjs` · `node scripts/verify-cp-auth.mjs` · `node scripts/verify-cp-registry-sqlite.mjs`
