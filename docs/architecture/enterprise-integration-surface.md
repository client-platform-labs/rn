...
>
> **决策（map-j/T6, #295 · 2026-09-14）：跟车窗口（production/next/minimum）作为文档决策。** 壳链发布面在企业侧（商店/MDM/跟车纪律），平台不建代码机制；等出现真实壳链跟车需求（多业务共壳排程 / 壳版本兼容窗口校验）再契约化。依据：无真实需求时建机制 = 为将来预留（防过度设计）；壳链发布面本就在企业手里（见上）。
