...
**触发**：#257 去重后 shell-core 提供默认的签名 CRL 拉取，因此**绿色壳现在也校验 CRL**（此前完全没有吊销校验）。需按行业实践定姿态。

### 行业依据

| 参照 | 吊销机制 | 对本平台的启示 |
| --- | --- | --- |
| **TUF / Uptane**（更新安全权威标准） | **不用 CRL/OCSP**。Root 角色重签/替换密钥实现吊销；元数据带 **expires**（freshness） | 吊销可靠地靠**密钥轮换 + 元数据有效期**，而非吊销清单可达性 |
| **移动 OTA 代码签名**（Expo EAS Update / CodePush） | 私钥签包、公钥**内嵌 app**、支持密钥轮换；无 CRL 语义 | 设备侧只认内嵌信任根 + 签名，不依赖在线吊销查询 |
| **Web PKI / 嵌入式** | CRL/OCSP 是标准手段；浏览器**普遍 soft-fail**；设备常**无 responder 路径**；行业方向是**短证书有效期**（Let's Encrypt CRL-first + 短生命周期） | 把更新可用性绑在吊销响应者可达性上，是已知反模式 |
| **硬失败实践** | dotnet SDK bootstrap 对**瞬时** CRL 失败 hard-fail；.NET/Linux X509 链在 CDP 不可达时 hard-fail | 校验吊销时 hard-fail 属常见实践 |

### 两个失败类必须区分（代码已区分）

1. **CRL 存在但未签名/验签失败/被篡改** → **hard-fail**。这是攻击信号，无歧义（#253 建立的语义，保持不变）。
2. **CRL 不可达**（404/5xx/网络/超时）→ 这是**可用性**事件，不是攻击信号。

### 决定：保留 fail-closed 的签名 CRL 校验

理由：

- **(a) 主流**：在"确实校验吊销"的系统中，对无效吊销数据 hard-fail 是多数实践。
- **(b) 拓扑上几乎不新增可用性面**：`manifest` 与 `/v1/crl` **同源、同一控制面**。CP 不可达时更新本来也拉不到 —— 常见故障模式下 CRL 不引入新的可用性风险。唯一新增耦合是"CP 对 manifest 可达但对 `/v1/crl` 不可达"（代理/CDN 未路由该路径、或 CP 版本早于该端点），这是**部署一致性要求**，非根本风险。
- **(c) soft-fail 会重开 G3**：若对"不可达"放宽，则能阻断 `/v1/crl` 的攻击者即可**清空吊销** —— 正是 #253 用签名 CRL 修掉的那个洞。

**部署前置条件（必须满足）**：控制面提供**已签名**的 `/v1/crl`（自 #253 成立），且与本壳**同批或先行**部署。

### 收敛方向（记录，不在本图实现）

行业两法可移除"每次启动依赖 CRL 可达"的耦合，同时保持吊销有效：

- **(i) 短有效期叶子证书** —— 使暴露窗口有界，避免依赖吊销清单；
- **(ii) 带 expiry 的缓存吊销清单**（TUF freshness）—— 接受缓存直到其 `nextUpdate`；既无新鲜也无缓存时才拒绝更新。

二者结合是 ADR-024 的后续演进方向，**待实测需要时再做**（YAGNI：当前同一 CP 的拓扑尚未证明该耦合造成实际故障）。
