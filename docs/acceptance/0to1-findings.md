...
| F13 | P1 初始化（D4 对齐） | S3 | init 产物不完整：`metro.config.js` require `.rn/metro/host-resolver.cjs`，但 init **不生成**该文件（由 `rn module register`/`rn dev` 才生成，`host-metro-config.ts:139`）→ 首次 `rn dev` 前 metro 会 warn 降级、平台包解析未接线 | 实机：`.rn/metro/` 仅有 main.config.cjs；host-metro-config.ts 注释"Regenerate …"；旧工程有该文件因跑过 register | 与 D4"init=最小可跑完整产品"不符；用户 init 后直接 dev 会遇 warn/解析缺失 | init 末尾应生成 host-resolver.cjs（或 init 内联执行 module register 等价步骤），消除"init 后不可直接 dev"的缝隙 | 待修复 |
