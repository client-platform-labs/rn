...
| F08 | P0 工具链 | S3 | 一键安装后"当前终端激活"步骤非一等公民：`curl \| bash` 跑在子 shell，无法改父 shell PATH（POSIX 约束，rustup/nvm 同款）；get-rn.sh 的激活提示（`source ~/.config/client-platform/rn-env.sh` 或重开终端）埋在 install OK 与 Lifecycle 之间易滚走；未检测当前 PATH 是否已含 LOCAL_BIN | 安装输出尾部；`ensure_path_profile` 的 export 仅作用于子 shell | 新用户装完当前终端不可用且提示不显眼——安装旅程体验差 | ① install OK 后第一行显眼提示（已含 LOCAL_BIN → "已可用"；未含 → "source … 或重开终端"）② cli-distribution.md 把激活步骤定为一等公民 | 待修复 |
