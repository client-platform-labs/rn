...
## 系统级根因与关联分析（替代点对点，先梳理再动手）

> 23 条 finding 不是 23 个独立问题，是 **5 个"架构接缝"（seam）缺失**的切片。修复必须以接缝为单位，不能逐条打补丁。
...
| **R4 工具链生命周期模型** | 安装/卸载/升级/启停：单一 home、可逆、幂等、语义一致 | F03 F07 F08 F12 | 高（数据/信任安全） |
| **R5 运行时身份校验** | 复用任何长驻基础设施前验证工程身份 | F16 | 中（防串线） |

### 关键关联（问题之间不是独立的）

1. **F23(S1) = R2 的枢纽切片**：与 F02（烘焙硬编码）、F13（resolver 非 init 生成）**同一根因**——平台没有"运行时配置统一生成/注入点"，所以基址、公钥、resolver 全缺或硬编码。**修 R2 一次，F02/F13/F14/F17/F18/F23 一起解。**
2. **信任根链条**：F01→F02→F05→F06。F06 已决策 B（证书链+HSM）会**级联改变** F01（keygen→HSM/证书）、F02（烘焙根 CA）、F04（吊销→CRL/OCSP）。
3. **生命周期链条**：F08（安装路径选错）→ F03（撞密钥目录/卸载删钥）→ F07（卸载不完整）→ F12（hoist 副作用路径失效）——全因无统一生命周期模型。
4. **内部代号泄漏三连**：F09（命令长/产品非默认）· F10（flag 泄漏 topology）· F11（输出泄漏 ADR）——同源"用户表面不诚实"。
5. **resolver 分裂族**：F17（dev/release 分裂）与 F13（init 不生成）同属"resolver 无单一生成点"；F18（apply-ota 死依赖）同属"工具脚本不走单事实源"。
6. **旧坑回填族**：F19（release 卫生）+ F20（网络安全）都是"此前 drill 已踩过、手工修过、但没回填模板"——R1 的直接例证。

### 系统级解决 = 补接缝，不是补 finding

每个 R 建一个**一等公民接缝**，含：接缝契约（应该长什么样）+ 收敛 finding + 实现工作项 + **验收探针**（保证同类问题不再复发）：

- **R1 接缝（契约-实现一致性）**：每个 ADR 承诺能力 = 一个可验证的生成物/默认（模板烘焙 + 验收探针）；"已踩过的坑必须回填模板"。探针：新工程跑验收链，旧 finding 对应的探针必须绿。
- **R2 接缝（声明→配置/派生物管线）**：公钥/CP基址/resolver/release卫生 = 单一事实源（manifest/配置）+ 统一生成/注入点（init 收尾/register/dev 预检）+ 质量门禁。探针：新工程 init 后 OTA 可直接配置使用（F23 探针）。
- **R3 接缝（用户表面契约）**：命令名/帮助/输出/字段语义化规则（不泄漏内部代号、指向真实产物、字段语义明确）。探针：命令输出扫描（无内部代号/无失效路径）。
- **R4 接缝（工具链生命周期模型）**：单一 home + 可逆 + 幂等（pi/rustup 模型）。探针：install→uninstall→reinstall 往返无残留（F03/F07 探针）。
- **R5 接缝（运行时身份校验原语）**：端口/进程/服务→工程身份，复用前强制校验。探针：跨工程复用被拒（F16 探针）。

### 开图（按接缝，替代 T1–T4 初稿）

| Ticket | 接缝 | 收敛 | 级联 |
|--------|------|------|------|
| **SEAM-1** | R1 契约-实现一致性 | F01 F04 F05 F06 F19 F20 | 依赖 SEAM-2（配置注入） |
| **SEAM-2** | R2 配置/派生物管线 | F02 F13 F14 F17 F18 F23 | 枢纽，先做 |
| **SEAM-3** | R3 用户表面契约 | F09 F10 F11 F12 F15 F21 F22 | 依赖 SEAM-2（输出真实产物） |
| **SEAM-4** | R4 工具链生命周期 | F03 F07 F08 F12 | 独立 |
| **SEAM-5** | R5 运行时身份校验 | F16 | 独立 |
