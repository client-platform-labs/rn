...
- DR 契约 = **冷重建**：每日加密异地备份（注册库 `sqlite .backup` + 制品 tar + **完整信任材料目录** `keys/`（cert 模式 RCA/leaf 私钥与证书、CSR、serial）+ 签名私钥 + `.env`），任意装 Docker 的机器十几分钟重建后恢复。
- 备份加密用 **age**：脚本用 age 公钥加密（公钥滚进脚本/仓库），解密私钥由人异地保管、不进 git、不上机；覆盖信任材料目录 + 注册库 + `.env`。
- RPO ≈ 每日备份；RTO ≈ 分钟级手动重建。
- 本地 Mac 仅作冷备/预发，不对设备提供服务。
- 私有不可再生项（RCA 私钥 = 设备烘焙的信任根、leaf 私钥、注册库）**必**加密异地备份；JS 包可重打。
  - **P2 修正（2026-09-14 演练）**：此前备份只捕获单个签名私钥，恢复后设备拒绝重签的 CRL（fail-closed 挡住更新）。现在备份包含**整个 keys 目录**（含 RCA 私钥），冷重建后即可重新签发设备可信的 CRL 与合法发布包 —— 演练终探针断言 `device accepts CRL = true`（恢复前 true → 恢复后 true）。
