...
export function renderModuleRegistry(
  ...
  const registrations = modules
    .map((mod) => {
      ...
      return `  registerModule({\n    moduleId: ${JSON.stringify(id)},\n    getApp: get${name}App,\n    loadSidecar: () => {\n      const live = (globalThis as Record<string, unknown>)["__CP_OTA_SIDECAR_${id}"];\n      if (live) return live;\n      return loadJson(require("./fixtures/modules/${id}/sidecar.json"));\n    },\n  });`;
    })
    .join("\n");
  return `/**
 ...
 */
import { registerModule, loadJson } from "./ModuleRegistry.js";
${imports}
